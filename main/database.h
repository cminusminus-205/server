...
public:
	...
	void wipe(){
		...
		*this << new Query("DROP TABLE IF EXISTS chat");
		...
		*this << new Query("CREATE TABLE emergencies (id INTEGER UNIQUE PRIMARY KEY, type TEXT NOT NULL, latitude TEXT, longitude TEXT, status TEXT NOT NULL, description TEXT NOT NULL, time TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, IS_SIMULATION TEXT);");
		...
		*this << new Query("CREATE TABLE chat (id INTEGER UNIQUE PRIMARY KEY, sent_by TEXT NOT NULL, message TEXT, time TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP);");
		...
		*this << new Query("INSERT INTO emergencies (type, latitude, longitude, status, description) VALUES ('TEST', '40.699190', '-75.211171', 'ACTIVE', 'Test, please ignore');");
